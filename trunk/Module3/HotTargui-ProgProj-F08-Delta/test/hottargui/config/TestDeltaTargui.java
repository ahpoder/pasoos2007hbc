...
public class TestDeltaTargui {
  ...
  public void testRedAttacksBlueAndLoose() {
	...
	((DeltaTile)t).changePlayerColor(PlayerColor.Blue);
	((DeltaTile)t).changeUnitCount(8);
  ...
  public void testCalculateRevenueForFourPlayers() {
		...
		((DeltaTile)t).changePlayerColor(PlayerColor.Green);
		// Erg to Blue = 1
		t = game.getTile(new Position(6,5));
		((DeltaTile)t).changePlayerColor(PlayerColor.Blue);
		// Reg to Yellow = 2
		t = game.getTile(new Position(5,4));
		((DeltaTile)t).changePlayerColor(PlayerColor.Yellow);
		// Fesh-fesh to Red = 0
